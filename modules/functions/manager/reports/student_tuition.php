    ...
    <div class="card mb-4">
        ...
        <table width="100%" class="display" id="datatable">
            ...
            <tbody>
                <tr>
                    <?php
                    $term_fees_query = mysqli_query($db, "SELECT students.name, SUM(amount), term, year, date_paid
                                    FROM fees
                                    LEFT JOIN students ON students.id = fees.student_id
                                    GROUP BY term, students.id") or die("An error occured: " . mysqli_error($db));

                    if (mysqli_num_rows($term_fees_query) > 0) {
                        $total_fees = 0;
                        $total_balance = 0;
                        while ($term_fees_row  = mysqli_fetch_assoc($term_fees_query)) {
                            $student_name = $term_fees_row['name'];
                            $total_amount_paid = $term_fees_row['SUM(amount)'];
                            $term = $term_fees_row['term'];
                            $year = $term_fees_row['year'];

                            $balance_due = $total_term_fees - $total_amount_paid;

                            $total_fees += $total_amount_paid;
                            $total_balance += $balance_due;


                            $date_raw = strtotime($term_fees_row['date_paid']);
                            $date = date('F d, Y', $date_raw);

                            echo "<td>" . $date . "</td>";
                            echo "<td>" . $term . " - " . $year . "</div></td>";
                            echo "<td>" . $student_name . "</td>";
                            echo "<td>" . number_format($total_amount_paid, 2) . "</td>";
                            echo "<td>" . number_format($balance_due, 2) . "</a></td>";
                    ?>
                </tr>
