...
<body>
    ...
    <div class="text-right text-light col-md-12">
        <br>
        <div class="btn-group">
            <button class="btn btn-sm btn-info" onclick="location.href='../accountant/expenses/index.php';">Back </i> </button>
            <a class="btn btn-sm btn-success" onClick="window.print()"> Print Expense Report <i class="fas fa-print"></i> </a>
        </div>
    </div>
    ...
    <div class="container"  id="print_it">
        ...
        <form id="contactus" action="" method="post" >
            ...
            <!-- Get Results -->
            <div class="text-center text-info"><h2> Expense Reciept </h2></div>
            <br><br>
                ...
                <div class="col-lg-10 mx-auto mb-4">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            ...
                            $expenses_query = mysqli_query($db, "SELECT * 
                                FROM expenses WHERE id = '$expense_id' " )
                                or die("An error occured: ".mysqli_error($db));
                            if(mysqli_num_rows($expenses_query) >= 1){
                            while($expenses_row  = mysqli_fetch_assoc($expenses_query) ){
                                $acc_name = $expenses_row['paid_by'];
                                $desc = $expenses_row['description'];
                                $amount_paid = $expenses_row['amount'];
                                $method = $expenses_row['method'];
                                $date_paid_raw = strtotime($expenses_row['date']);
                                $date_paid = date('d F, Y',$date_paid_raw);

                                /** Set expense vars */
                                if($expenses_row['method'] == "Cash"){
                                    $acc = $expenses_row['recieved_by'];
                                    ...
                                    while($reciever_row  = mysqli_fetch_assoc($reciever_query) ){
                                        ...
                                        $expense_method = " in cash recieved by $reciever ";
                                    } }
                                }elseif($expenses_row['method'] == "Bank"){
                                    $bank = $expenses_row['bank_acc'];
                                    ...
                                    while($reciever_row  = mysqli_fetch_assoc($reciever_query) ){
                                        ...
                                        $expense_method = " via a bank deposite from $bank_acc $bank_name ";
                                    } }
                                }else{ $expense_method = " via ".$expenses_row['method']; }

                                echo "<p> <b>$acc_name </b> has on <b> $date_paid </b> authorised the sum of <b> $currency ". number_format($amount_paid, 2) ." </b><b> $expense_method </b>
                                        for <b> $desc </b> </p>";
